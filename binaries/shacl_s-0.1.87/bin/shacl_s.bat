@REM shacl_s launcher script
@REM
@REM Environment:
@REM JAVA_HOME - location of a JDK home dir (optional if java on path)
@REM CFG_OPTS  - JVM options (optional)
@REM Configuration:
@REM SHACL_S_config.txt found in the SHACL_S_HOME.
...
if "%SHACL_S_HOME%"=="" (
  ...
  set "SHACL_S_HOME=%~dp0\\.."
) else (
  set "APP_HOME=%SHACL_S_HOME%"
)
...
set "CFG_FILE=%APP_HOME%\SHACL_S_config.txt"
set CFG_OPTS=
...
set "APP_CLASSPATH="
set "APP_MAIN_CLASS=-jar "%APP_LIB_DIR%\es.weso.shacl_s-0.1.87-launcher.jar""
...
if "%JAVAOK%"=="false" (
  ...
  echo and download a valid Java JDK and install before running shacl_s.
  echo.
...
"%_JAVACMD%" !_JAVA_OPTS! !SHACL_S_OPTS! -cp "%APP_CLASSPATH%" %MAIN_CLASS% !_APP_ARGS!
